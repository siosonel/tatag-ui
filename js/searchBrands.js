function searchBrands(api) {
	...
	function renderBrandDiv(brand) {
		...
		$('#brandsWrapper').append(
			...
			+ 		"<div class='small-9 columns'>"+ brand.mission +"</div>"
			+		"</div>"
			+		"<div class='row'>"
			+			"<div class='small-3 columns'>Description:</div>"
			+ 		"<div class='small-9 columns'>"+ brand.description +"</div>"
			+		"</div>"
			+		"<div class='row'>"
			+			"<div class='small-3 columns'>URL:</div>"
			+ 		"<div class='small-9 columns'>"+ brand.url +"</div>"
			+		"</div>"
			//+		"<div class='row'>"
			//+			"<div class='small-3 columns'>Advisor:</div>"
			//+ 		"<div class='small-9 columns'>"+ brand.advisor +"</div>"
			//+		"</div>"
			+		"<div class='row'>"
			+			"<div class='small-3 columns'>Type:</div>"
			+ 		"<div class='small-9 columns'>"+ brand.type_system +": "+ type +"</div>"
			+		"</div>"
			+		"<div class='row'>"
			+			"<div class='small-3 columns'>Location:</div>"
			+ 		"<div class='small-9 columns'>"+ byIso3[brand.country_code][0] +": "+ brand.area_name +"</div>"
			+		"</div>"
			+		"<div class='row'>"
			+			"<div class='small-3 columns'>ID#:</div>"
			+ 		"<div class='small-9 columns'>"+ brand.brand_id +"</div>"
			+		"</div>"
			+'</div>'
		);
	}
